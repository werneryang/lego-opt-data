...
def create_test_snapshot_with_errors(test_dir: Path, trade_date: date):
    ...
    logger.info(f"  - Successful rows: 1")
    logger.info(f"  - Error rows: 2")
...
def test_rollup_handles_errors(cfg, test_dir: Path, trade_date: date):
    ...
    try:
        ...
        logger.info(f"Rollup completed:")
...
def test_qa_metrics_account_for_errors(test_dir: Path, trade_date: date):
    ...
    if "data_quality_flag" in df.columns:
        flagged_rows = df[df["data_quality_flag"].apply(lambda x: len(x) > 0 if isinstance(x, list) else False)]
...
def test_data_quality_propagation(test_dir: Path, trade_date: date):
    ...
    error_rows = df[df["snapshot_error"] == True]
    ...
    for idx, row in error_rows.iterrows():
        ...
        if all_none:
            logger.info(f"  ✓ All market data fields are None/NaN")
        else:
            logger.warning(f"  ✗ Some market data fields have values")
