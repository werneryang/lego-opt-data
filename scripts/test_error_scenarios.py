    ...
    try:
        ...
        for result in results:
            ...
            if result.get("snapshot_error"):
                logger.info("✓ Error correctly flagged")
                logger.info(f"  Error type: {result.get('error_type')}")
                logger.info(f"  Error message: {result.get('error_message')}")
            else:
                logger.warning(
                    f"✗ Expected snapshot_error=True but got {result.get('snapshot_error')}"
                )
    ...
    try:
        ...
        for result in results:
            if result.get("snapshot_timed_out"):
                logger.info("✓ Timeout correctly detected")
                ...
                logger.info("✓ Error flagged (might be timeout or other error)")
                logger.info(f"  Error type: {result.get('error_type')}")
            else:
                logger.info("Data was ready within timeout - try shorter timeout")
    ...
    try:
        ...
        for i, result in enumerate(results):
            ...
            if result.get("snapshot_error"):
                ...
                logger.info(
                    f"  [{i}] {symbol}: SUCCESS - bid={result.get('bid')}, ask={result.get('ask')}"
                )
