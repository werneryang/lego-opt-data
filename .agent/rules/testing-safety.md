...
- **IB 安全与限速**: 统一用 `ib_insync`（禁止直接 `ibapi`），默认 `IB_HOST=127.0.0.1`, `IB_PORT=7497`, `IB_CLIENT_ID=101`，市场数据类型与权限在测试前确认。遵循 pacing：snapshot 30 req/min + 并发 10（可调）；发现阶段依赖 IB pacing 不自限。
