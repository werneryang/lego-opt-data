...
## 盘中实时订阅（新增）
- 目标：交易时段新增 SPY 盘中实时订阅；订阅项含 SPY Spot、SPY 5s Bars、VIX Spot；期权合约为 2 个到期日（本周五 + 下个月月度）× 20 个行权价（ATM 上下各 10）= 40 条。
- 可扩展性：`underlyings` 可配置，后续扩展 QQQ/IWM；到期日策略与行权价范围可配置。
- 动态调整（Hysteresis）：标的价格移动超过 3 个行权价间隔触发重平衡；计算新 ATM 与 20 个行权价范围；取消滚出范围的 6 个合约（两侧各 3）；订阅新进入范围的 6 个合约；阈值与步进以期权链实际行权价间隔为准（SPY 通常 $1）。
- 行情字段：如性能允许，订阅 `bid/ask/last/iv/greeks`；记录缺失字段与延迟标记，便于 QA。
- 运行与健壮性：订阅数量与 pacing 监控、断线重连、配置化限速；变更订阅时保证幂等与最小增删。
- 运行策略：交易日 09:35–16:00 ET 无人值守运行，收盘后退出；由 launchd/systemd timer/cron 定时拉起 `opt_data.cli streaming` 并使用 `--duration` 控制运行窗口。
- 存储与对外：定义实时流落盘路径与 schema（含时间戳、合约 id、字段来源），与现有 snapshot/rollup 数据区分。
