...
if [[ "$REMOTE_EXIT" -ne 0 ]]; then
  if grep -E "cd: .*No such file or directory" "$LOCAL_STDOUT_LOG" >/dev/null 2>&1; then
    echo "[local] remote repo path not found; re-run with --repo <actual remote path>"
  fi
fi
...
set -euo pipefail
shopt -s nullglob
ls -t "$REMOTE_REPO/state/run_logs/ops_checks"/prod_schedule_check_*.log 2>/dev/null | head -n 1
EOF
    )
    REPORT_PATH="$("${SSH_BASE[@]}" "${USER}@${HOST}" "bash -lc $(printf '%q' "$PROBE_CMD_STR")")"
  else
