...
def mcp_server(
    config: Optional[str] = typer.Option(None, help="Path to config TOML"),
    audit_db: Optional[str] = typer.Option(
        None, help="Path to MCP audit SQLite DB (default: state/run_logs/mcp_audit.db)"
    ),
    allow_raw: bool = typer.Option(True, help="Allow read access to data/raw"),
    allow_clean: bool = typer.Option(True, help="Allow read access to data/clean"),
    log_level: str = typer.Option("INFO", help="Logging level"),
) -> None:
    """Run MCP server over stdio for read-only analysis tools."""
    cfg = load_config(Path(config) if config else None)
    logging.basicConfig(level=log_level.upper())

    audit_path = Path(audit_db) if audit_db else None
