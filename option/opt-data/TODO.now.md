...
- [x] MCP 运行状态信号增强：从 state/run_logs 汇总简化状态视图（不新增 DB）。（2025-12-25）
- [x] MCP 配置化限制与白名单：mcp limit/days/allow_raw/allow_clean 并在 server 读取。（2025-12-25）
- [x] MCP 输出一致性与自检：工具返回 meta（limit/clamped/source）并补充轻量断言。（2025-12-25）
