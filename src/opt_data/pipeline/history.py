...
class HistoryRunner:
    ...
    def run(
        ...
        results = {"processed": 0, "errors": 0, "symbols": {}}
            ...
            for i, symbol in enumerate(symbols):
                ...
                try:
                    ...
                    if incremental:
                        ...
                        if symbol_dir.exists():
                            ...
                            existing_dates = sorted(
                                [d.name for d in symbol_dir.iterdir() if d.is_dir()]
                            )
                            if existing_dates:
                                ...
                                try:
                                    ...
                                    if gap_days <= 0:
                                        logger.info(
                                            f"Skipping {symbol}: Up to date (last: {last_date})"
                                        )
                                        if progress_callback:
                                            progress_callback(
                                                i + 1,
                                                total_symbols,
                                                f"Skipped {symbol} (Up to date)",
                                                {},
                                            )
                                        continue
                                    ...
                                    logger.info(
                                        f"Incremental fetch for {symbol}: {duration_str} (last: {last_date})"
                                    )
                                except ValueError:
                                    pass
                    ...
                    if bars:
                        ...
                        logger.warning(
                            f"Could not fetch reference price for {symbol}; using 0.0 (discovery may fail)"
                        )
                    ...
                    if progress_callback:
                        progress_callback(
                            i + 1, total_symbols, f"Failed {symbol}: {exc}", {"error": str(exc)}
                        )
